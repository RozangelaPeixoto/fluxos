...
class _WorkOrderFormScreenState extends State<WorkOrderFormScreen> {
  ...
  void initState() {
    ...
    final nFmt = NumberFormat.currency(locale: 'pt_BR', symbol: '', decimalDigits: 2);
    ...
    if (widget.os != null && widget.os!.photos.isNotEmpty) {
      _photos = widget.os!.photos.split(',').where((p) => p.isNotEmpty).toList();
    }
  ...
  bool _validateStatusTransition() {
    ...
    if (_status == 'Aguardando peça' && _diagnosisController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Para o status "Aguardando peça", um diagnóstico é obrigatório.'), backgroundColor: Colors.red));
      return false;
    }
    if (_status == 'Concluída' && _solutionController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Para o status "Concluída", uma solução proposta é obrigatória.'), backgroundColor: Colors.red));
      return false;
    }
    return true;
  }
  ...
  void _save() {
    if (_formKey.currentState!.validate()) {
      ...
      final newOs = WorkOrder(
        ...
        deadline: _deadlineController.text.isEmpty ? null : _deadlineController.text,
        status: _status,
  ...
  Widget _buildSectionTitle(String title) {
    return Padding(
      ...
      child: Text(title, style: const TextStyle(fontSize: 18, color: Colors.black87, fontWeight: FontWeight.bold)),
    );
  }
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Problema relatado', style: TextStyle(color: Colors.black54)),
              const SizedBox(height: 8),
              TextFormField(
              ...
              Wrap(
                ...
                children: ['Baixa', 'Média', 'Alta', 'Urgente'].map((p) {
                  ...
                  return ChoiceChip(
                    ...
                    selectedColor: p == 'Baixa' ? Colors.green.shade100 :
                                   p == 'Média' ? Colors.blue.shade100 :
                                   p == 'Alta' ? Colors.orange.shade100 : Colors.red.shade100,
                    labelStyle: TextStyle(
                      color: isSelected ? 
                        (p == 'Baixa' ? Colors.green.shade800 :
                         p == 'Média' ? Colors.blue.shade800 :
                         p == 'Alta' ? Colors.orange.shade800 : Colors.red.shade800)
                      : Colors.black87,
              ...
              DropdownButtonFormField<String>(
                value: _technicianId,
                decoration: _fieldDeco(),
                items: activeTechs.map<DropdownMenuItem<String>>((t) => DropdownMenuItem<String>(value: t.id, child: Text(t.name))).toList(),
              ...
              const Text('Diagnóstico', style: TextStyle(color: Colors.black54)),
              const SizedBox(height: 8),
              TextFormField(
              ...
              const SizedBox(height: 16),
              const Text('Solução proposta', style: TextStyle(color: Colors.black54)),
              const SizedBox(height: 8),
              TextFormField(
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(color: Colors.grey.shade400, style: BorderStyle.solid), // Ideally dashed, but solid works as fallback
                    ),
                    ...
                    children: [
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: const Center(child: Icon(Icons.add, color: Colors.grey, size: 32)),
                        ),
                      )
                    ],
                  ),
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text('R\$ ${NumberFormat.currency(locale: 'pt_BR', symbol: '', decimalDigits: 2).format(_totalCost)}', style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
                  ],
                ),
              ),
